...
def smoke(account_id, sub_id, alice_id, start):
    ...
    with sync_playwright() as playwright:
        ...
        try:
            ...
            page.get_by_role("link", name="请求明细", exact=True).click()
            expect(page.get_by_role("button", name="查看请求 request-1", exact=True)).to_be_visible()
            ...
            rider.goto("http://127.0.0.1:5179/cpa-requests")
            requests = rider.get_by_test_id("cpa-requests")
            expect(requests).to_contain_text("2 条")
            expect(rider.get_by_role("button", name="查看请求 request-1", exact=True)).to_have_count(0)
            expect(rider.get_by_role("button", name="查看请求 request-0", exact=True)).to_be_visible()
            expect(rider.get_by_role("button", name="查看请求 request-2", exact=True)).to_be_visible()
            rider.get_by_label("请求状态", exact=True).select_option("true")
            rider.get_by_role("button", name="查询", exact=True).click()
            expect(requests).to_contain_text("0 条")
            rider.get_by_label("请求状态", exact=True).select_option("")
            rider.get_by_role("button", name="查询", exact=True).click()
            expect(requests).to_contain_text("2 条")
