...
- 系统设置提供默认开启的“自动应用建议额度”：复用采样策略中的本地探测间隔，在每轮定时采样结束后，仅应用当前完整且待应用的 Sub2API 聚合余额建议；没有建议、已应用或暂停后台监控时不写余额，与手动应用共用并发锁及幂等操作日志。如不需要，可在系统设置中关闭。
- 建议余额按同期实际扣费与测算成本之比换算；无个人消费样本时采用账号样本，各账号带符号换算后再合计。页面、通知和应用使用同一钱包金额；历史使用冻结修正规则，新采样不再叠加本地修正，详见 [余额建议换算](docs/billing-corrections.md#余额建议的扣费口径换算)。
- 首页提供“临时爽蹬”：本期全局建议余额 9999，首个账号换周期后自动退出；按超用量与其他人的未用份额比例结转借用权益，闲置额度不无限累积。支持原有手动/自动应用，并提供独立的[图示教程与结算示例](docs/temporary-burst.md)。
- 上游整数百分比、观测时间、周期边界和已采集余额永久保留；新采样按 canonical point 原子提交，历史维护先冻结不可变的本地审计计划，再零联网应用并确定性重放派生结果。
- “Sub2API 上游计费”由管理员手动选择目标分组，再写入 FAST、模型价格与长上下文开关；写后读取确认，支持失败重试和恢复接管前值。未选分组不自动写入，旧本地规则与历史保持冻结；新观测保留原始事实，确认后显示“已修正”，撤回不会重启本地修正。详见 [计费配置与升级说明](docs/billing-corrections.md)。
...
> “自动应用建议额度”默认开启；升级迁移会为已有安装一次性开启，之后手动关闭的设置会保留。管理员也可手动点击“一键设置”。使用共享订阅或相关网关前，请自行确认上游服务条款及所在地法律要求。
