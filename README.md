...
- 建议余额按参与者在对应账号、当前测算区间的实际扣费与修正成本之比换算；无个人消费样本时采用账号样本，各账号带符号换算后再合计。页面、通知和应用使用同一钱包金额，模型容量与百分比推断不变。历史使用结构是估计，优先建议在 Sub2API 统一倍率并避免本地重复修正，详见 [余额建议换算](docs/billing-corrections.md#余额建议的扣费口径换算)。
