...
管理员可在系统设置的“科研共创”卡片中选择 **GPT-6 额度异常归因**。独立 `runresearch` 进程定时从本地原始额度快照与成本分项进行比较，不使用当前修正后的金额或粒子滤波结论充当真值，不额外调用模型，不改参与者额度。

默认接收地址 `https://study.example.invalid` 是占位，不发出网络请求。部署 [CodexSubscribeStudy](https://github.com/LingyeNBird/CodexSubscribeStudy) 后填写其HTTPS根地址，阅读发送字段与隐私边界，再明确确认授权。更换网站、研究范围或方法需要重新同意；数据库导入后自动关闭分享。关闭停止新发送，可另行撤回已提交的本安装摘要。

发送仅限汇总请求/成本/额度、质量计数、候选评分均值/协方差、探索性支持度与随机公钥版本；不含明细、姓名、API Key或IP字段。**直接连接的接收服务器/代理仍能看到出口IP**，不能承诺网络层完全匿名。公开统计只按随机安装键去重，不能证明独立真人或数据真实性。

v1仅研究普通档位、非长上下文、GPT-5.6/GPT-6混合且原始事实覆盖完整的区间；分项仅在同意后保存，旧缺失数据不猜测。至少2周期/24块/200请求及用量覆盖确认后才尝试归因。支持度不是平台真实机制的概率。详见[科研方法与限制](docs/research-method.md)。迁移0050新增独立授权表和原始分项sidecar，不改变已有成本事实或计费规则。
