...
- 上游整数百分比、观测时间、周期边界和已采集余额永久保留；新采样按 canonical point 原子提交，历史维护先冻结不可变 plan 与逐维 coverage，再零联网 apply 和确定性重放。
- 可选校正 FAST 请求的计费倍率差异；历史 FAST 修复只能与账号、逐用户成本一起通过 verified remote plan 原子采用，分页一致或查询天数不足以授权覆盖。
