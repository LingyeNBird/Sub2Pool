...
定时任务使用本地已捕获的原始事实，不额外请求模型或从上游回抓。按匿名历史批次更新，重复不加量；不同批次持续保留，无120天自动清理或导入自动撤回。关闭只停止发送；撤回需管理员单独确认。缺少额度/成本证据时保留请求统计并说明缺失，不编造归因。

只上传汇总计数、候选证据曲线、信息矩阵和候选归因合计，不上传请求明细、账号/参与者、Token明细、API Key、时间线或IP字段。随机公钥可关联更新，不是绝对匿名；单条贡献无k匿名保证，网络层仍可见出口IP。

Docker入口运行独立 `runresearch` 进程。非Docker须另外托管 `python manage.py runresearch` 或定期 `--once`。方法、升级和原始数据边界见 [原始证据共研说明](docs/research-v2.md)。
