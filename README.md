...
- Sub2API 0.1.179 起推荐直接在 OpenAI OAuth 渠道配置 2.5 倍 FAST 计费；Sub2Pool 默认启用 2 倍到 2.5 倍的兼容修正，并支持按模型配置有序通配规则，已保存的历史修正事实仍会永久保留并参与重放。
...
- 提供额度总览、参与者管理、可视化额度分配、观测记录、额度统计、登录审计和使用教程。
- 管理员可为普通系统用户逐页配置只读访问权限，并精确选择其可见账号与参与者；账号范围同时约束额度总览、账号状态、观测、粒子轨迹和统计，所有页面写操作仍仅限管理员。
- 提供内置 OpenAPI 3.1 文档的业务 API；管理员全局 Bearer Key 拥有全部已开放权限并可一键设置建议余额，普通系统用户也可生成绑定自身的 Key，其端点发现、OpenAPI 文档和业务响应实时继承页面、账号与参与者权限，且不能执行管理写操作。
