...
- 管理员可为普通系统用户逐页配置只读访问权限，并精确选择其可见账号与参与者；所有页面写操作仍仅限管理员。
- 提供内置 OpenAPI 3.1 文档的业务 API；管理员生成的永久 Bearer Key 默认拥有全部已开放 API 权限，可读取账号、额度、待应用建议明细、参与者、观测、模型、统计和通知，也可一键设置建议余额。
