...
- 原始采样永久保留；新增观测只计算自身，排除、恢复或设置起点时仅从最早受影响区间向后重算，不维护不可逆的周期状态。
- 可选校正 FAST 请求在 Sub2API 与上游套餐之间的计费倍率差异；修正事实按采样区间和原始用户 ID 保存，并支持从当前周期或全部历史重建。
- 平均恒定模型把上游整数进度视为截尾区间，并以余额范围展示不确定性；一键设置时采用范围中间值，时变额度模型仍输出单一建议值。
