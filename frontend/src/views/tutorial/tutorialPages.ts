        ...
        sections: [
          ...
          },
          {
            ...
            paragraphs: [
              ...
              "页面权限决定普通用户能进入哪些功能；账号和参与者权限继续过滤对应页面与 API 中的数据。普通用户获得的页面均为只读，不能触发测算、调额、编辑、封禁或维护操作。",
              "拥有“系统设置”权限的普通用户可在自己的设置页面生成绑定当前账号的 API Key；该 Key 只暴露已授权页面对应的只读端点，并沿用相同的账号和参与者范围。",
            ],
          },
        ],
        ...
        sections: [
          {
            title: "生成和保存 API Key",
            steps: [
              "管理员或拥有“系统设置”页面权限的普通系统用户进入“系统设置 → API”，点击“生成 API Key”。",
