  ...
  },
  {
    ...
    pages: [
      ...
      },
      {
        ...
        title: "数据维护与排错",
        summary:
          "从请求日志重建历史用量、重新生成计算结果、迁移数据库，并处理常见空数据问题。",
        icon: "wrench-screwdriver",
        sections: [
          {
            title: "历史数据全量重建",
            paragraphs: [
              "旧版本可能保存了不完整或错误的历史用量。进入“系统设置 → 数据维护”先检查可重建范围，再只读 Sub2API 请求日志，重新获取各时间区间的总用量、逐用户用量和 FAST 明细。",
              "重建会保留无法从日志恢复的上游整数百分比、观测时间、周期边界、人工操作记录和已保存的历史余额；其余用量事实与粒子结果全部覆盖重算。它不会向 Sub2API 写入任何数据。",
            ],
          },
          {
            title: "数据库迁移",
