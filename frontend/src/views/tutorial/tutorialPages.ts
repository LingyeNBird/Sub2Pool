        ...
        sections: [
          ...
          },
          {
            ...
            steps: [
              ...
              "如需让车友查看数据，创建普通系统用户，再分别配置可访问页面、账号状态页可见账号和其他页面可见参与者。",
      ...
      },
      {
        ...
        sections: [
          ...
          },
          {
            title: "创建普通系统用户",
            paragraphs: [
              "需要让车友自行查看时，由管理员先在“系统用户”页面创建登录账号，再点击该用户右侧的“编辑权限”。账号状态页面需要单独勾选可见的上游账号。",
              "页面权限决定普通用户能进入哪些功能；账号权限只允许查看勾选账号的运行和额度状态；参与者权限会统一过滤额度总览、参与者、观测记录、额度统计和通知记录中的具体参与者。普通用户获得的页面均为只读，不能触发测算、调额、编辑、封禁或维护操作。",
            ],
          },
        ],
  ...
  --output sub2pool-openapi.json`,
          ...
          },
          {
            ...
            paragraphs: [
              ...
              "GET /api/v1/account-status 会对每个监控账号执行 Sub2API 只读查询，返回运行状态、额度窗口和 30 天统计；FAST 修正总额来自本地已持久化事实。单个账号失败会写入该账号的 warnings，不会中断其他账号。",
            ],
            codeBlocks: [
              {
                title: "发现账号并读取指定账号总览",
