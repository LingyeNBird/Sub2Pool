...
const steps = [
  ...
  },
  {
    ...
    text: "额度统计页面会并列展示“本周期累计折算”和“今日用量折算”。今日观测跨过的整数周限不足设置阈值时会明确显示样本不足，不会用短增量给出误导结论；下方仍可按天、按月查看累计估算的收盘历史。",
  },
];
</script>
...
<template>
  ...
  <section class="card col-span-12 bg-base-200 shadow-xs xl:col-span-6">
    <div class="card-body gap-3">
      ...
      <p class="text-sm leading-6 opacity-70">
        系统不保存独立的周期账本。每次新增、排除或恢复观测时，都会按时间顺序重放该账号的全部原始采样：
        正常七天重置由新的 reset_at 识别；同一 reset_at
        下的单次百分比回退先按异常点排除，只有连续独立快照确认回退后才作为人工刷新边界。
        被排除记录始终保留作审计，但不会参与折算或归属。新边界形成有效样本前沿用上一段最终估值；
        只有账号从未形成有效历史样本时，才使用设置中的“无样本时美元 / 1%”。
      </p>
    </div>
  </section>
