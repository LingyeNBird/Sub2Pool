...
def withdraw():
    ...
    try:
        ...
        if type(ack.get("revision")) is not int or ack["revision"] != config.report_revision:
    ...
    with transaction.atomic():
        ...
        if current.config_revision == config.config_revision:
            current.last_sent_hash, current.last_sent_endpoint = "", ""
            current.last_status, current.last_error = "withdrawn", ""
            current.save()
            # Keep original local sources and batches, only invalidate receipts
            # after acknowledged deletion. Re-consent can contribute them again.
            ResearchEvidenceBatch.objects.update(sent_hashes={})
    return "withdrawn"
